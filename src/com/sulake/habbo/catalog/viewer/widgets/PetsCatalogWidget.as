package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_462 >= var_336.length)
         {
            return "";
         }
         var petColor:IPetColor = var_336[var_462] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_668[var_1682] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_462 = param1.index;
         if(var_462 < 0 || var_462 > var_336.length)
         {
            var_462 = 0;
         }
         updateImage();
      ...
      override public function init() : void
      {
         ...
         var_981 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_336 = [];
         var_668 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_981,this);
               if(_loc11_)
               {
                  var_668 = _loc11_.breeds;
                  var_336 = _loc11_.colors;
                  if(var_336 == null)
                  {
                     var_336 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_668.length)
         {
            _loc12_ = var_668[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_981,_loc12_.id),getRaceLocalizationKey(var_981,_loc12_.id)));
            _loc6_++;
         }
         var_608.add(var_981,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_1040,onApproveNameResult);
      ...
      private function updateImage() : void
      {
         ...
         if(var_462 >= 0 && var_462 < var_336.length)
         {
            _loc7_ = var_336[var_462] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_668[var_1682] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_981,_loc3_.id,_loc2_,AvatarScaleType.const_54,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_286,AvatarAction.const_628);

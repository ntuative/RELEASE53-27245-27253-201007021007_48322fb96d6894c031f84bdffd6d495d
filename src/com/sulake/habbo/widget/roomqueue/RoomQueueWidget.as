package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(var_1570 == RoomWidgetRoomQueueUpdateEvent.const_360)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_624);
         }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            var_1570 = param1.type;
            var_1084 = false;
            var_1083 = param1.position;
         }
         else
         {
            var_1084 = true;
         }
         var_1985 = param1.hasHabboClub;
         var_1569 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",var_1083.toString());
         localizations.registerParameter("room.queue.position.hc","position",var_1083.toString());
         localizations.registerParameter("room.queue.spectator.position","position",var_1083.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",var_1083.toString());
         showInterface();
      }
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_492,onQueueStatus);
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_579);
         messageListener.processWidgetMessage(_loc2_);
         removeWindow();
      }
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_492,onQueueStatus);
      }
         ...
         if(_loc1_ != null && _loc2_ != null && _loc3_ != null)
         {
            switch(var_1570)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_360:
                  _loc1_.caption = !!var_1569 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = var_1084;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_492:
                  _loc1_.caption = !!var_1569 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = var_1084;
         }
         var _loc4_:IWindowContainer = _window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !var_1985;
         }
